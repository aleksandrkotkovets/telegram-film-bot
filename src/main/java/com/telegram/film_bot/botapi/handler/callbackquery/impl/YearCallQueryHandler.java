...
@Component
public class YearCallQueryHandler implements CallbackQueryHandler {
    ...
    public YearCallQueryHandler(UserDataCache userDataCache,
                                Keyboard keyboard,
    ...
    private BotApiMethod<?> processUsersInput(CallbackQuery callbackQuery) {
        ...
        switch (usersCurrentBotState) {
            case RANDOM_FILM_SHOW_ASK_PARAM_KEYBOARD: {
                userDataCache.setUsersCurrentBotState(userId, BotState.RANDOM_FILM_CHOOSE_YEAR);
                SendMessage sendMessage = findFilmMessageService.getFindFilmMessage(chatId, "reply.chooseYear", Emoji.ARROW_DOWN,Emoji.ARROW_DOWN);
                List<String> yearList = filmService.getUniqParamList(CallbackQueryType.YEAR);
                sendMessage.setReplyMarkup(keyboard.getParamsKeyboard(yearList));
