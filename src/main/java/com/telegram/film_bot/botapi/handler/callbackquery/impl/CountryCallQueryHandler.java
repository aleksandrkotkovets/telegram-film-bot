...
@Component
public class CountryCallQueryHandler implements CallbackQueryHandler {
    ...
    public CountryCallQueryHandler(UserDataCache userDataCache,
                                 Keyboard keyboard,
    ...
    public CallbackQueryType getHandlerQueryType() {
        return CallbackQueryType.COUNTRY;
    }
    ...
    private BotApiMethod<?> processUsersInput(CallbackQuery callbackQuery) {
        ...
        switch (usersCurrentBotState) {
            case RANDOM_FILM_SHOW_ASK_PARAM_KEYBOARD: {
                userDataCache.setUsersCurrentBotState(userId, BotState.RANDOM_FILM_CHOOSE_COUNTRY);
                SendMessage sendMessage = findFilmMessageService.getFindFilmMessage(chatId, "reply.chooseCountry", Emoji.ARROW_DOWN, Emoji.ARROW_DOWN);
                List<String> countryList = filmService.getUniqParamList(CallbackQueryType.COUNTRY);
                sendMessage.setReplyMarkup(keyboard.getParamsKeyboard(countryList));
